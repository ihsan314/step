...
public final class FindMeetingQueryTest {
  ...
  public void optionalAttendeeCannotAttend() {
    // Have each person have different events, with one optional attendee having a day-long event.
    // We should see three options because each required attendee has split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    // Events  : |--------------C--------------|
    // Day     : |-----------------------------|
  ...
  public void optionalAttendeeIsConsidered() {
    // Have each person have different events, with one optional attendee having an event in the
    // middle.
    // We should see two options because each required attendee has split the restricted times.
    //
    // Events  :       |--A--|     |--B--|
    // Events  :             |--C--|
    // Day     : |-----------------------------|
  ...
  public void justEnoughRoomForRequiredAttendees() {
    // Have one required person and one optional person, but make it so that there is just
  ...
  public void optionalAttendeesWithGaps() {
    // Have two optional attendees for a meeting, but with sufficient gaps in the day. Meeting times
    // betweeen the events should be returned.
    // Events:          |-A-|   |-B-|
    // Day:     |-------------------------|
    // Options: |---1---|   |-2-|   |--3--|
    Collection<Event> events =
        Arrays.asList(
            ...
            new Event(
                "Event 2",
                TimeRange.fromStartDuration(TIME_1130AM, DURATION_30_MINUTES),
                Arrays.asList(PERSON_B)));
    ...
    Collection<TimeRange> expected =
        Arrays.asList(
            TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            TimeRange.fromStartEnd(TIME_0900AM, TIME_1130AM, false),
            TimeRange.fromStartEnd(TIME_1200, TimeRange.END_OF_DAY, true));
    Collection<TimeRange> actual = query.query(events, request);
    Assert.assertEquals(expected, actual);
  }
  ...
  public void optionalAttendeesButNoGaps() {
    ...
    Collection<Event> events =
        Arrays.asList(
            new Event(
                "Event 1",
                TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_1200, false),
                Arrays.asList(PERSON_A)),
            new Event(
                "Event 2",
                TimeRange.fromStartEnd(TIME_1200, TimeRange.END_OF_DAY, true),
